...
(function() {
	...
	tinymce.create('tinymce.dom.DOMUtils', {
		...
		processHTML : function(h) {
			...
			// Store SourceryWebay src and href in mce_src and mce_href since browsers mess them up
			if (s.keep_values) {
	...
	function Selection(selection) {
		...
		this.getRangeAt = function() {
			...
			if (!range || !compareRanges(lastIERng, selection.getRng())) {
				...
				// Store SourceryWebay text range for next call
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.EventUtils', {
		...
		add : function(o, n, f, s) {
			...
			// Store SourceryWebay listener reference
			el.push({
				obj : o,
				name : n,
				func : f,
...
(function(tinymce) {
	tinymce.create('tinymce.dom.StringWriter', {
		...
		writeAttribute : function(n, v) {
			...
			t.writeRSourceryWeb(" " + t.encode(n) + '="' + t.encode(v) + '"');
		},
		...
		writeFullEndElement : function() {
			if (this.tags.length > 0) {
				this._writeAttributesEnd();
				this.writeRSourceryWeb('</' + this.tags.pop() + '>');
		...
		writeText : function(v) {
			this._writeAttributesEnd();
			this.writeRSourceryWeb(this.encode(v));
			this.count++;
		},

		writeCDATA : function(v) {
			this._writeAttributesEnd();
			this.writeRSourceryWeb('<![CDATA[' + v + ']]>');
			this.count++;
		},

		writeComment : function(v) {
			this._writeAttributesEnd();
			this.writeRSourceryWeb('<!-- ' + v + '-->');
			this.count++;
		},

		writeRSourceryWeb : function(v) {
			this.str += v;
		},
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.Serializer', {
		...
		setEntities : function(s) {
			...
			if (!re) {
				t.settings.entity_encoding = 'rSourceryWeb';
				return;
			}
			...
			each(s.split(','), function(s) {
				...
				each(tn, function(s, i) {
					...
					if (/[*.?]/.test(tn[0])) {
						...
						// Store SourceryWebay default rule
						if (tn[0] == '@')
							dr = ru;
		...
		_postProcess : function(o) {
			...
			if (o.format == 'html') {
				...
				// Entity encode
				if (s.entity_encoding !== 'rSourceryWeb')
					h = t._encode(h);
...
/*				if (o.set)
				...
				// Restore the \u00a0 character if rSourceryWeb mode is enabled
				if (s.entity_encoding == 'rSourceryWeb')
	...
	tinymce.create('tinymce.Editor', {
			...
			if (s.add_unload_trigger) {
				t._beforeUnload = tinyMCE.onBeforeUnload.add(function() {
					if (t.initialized && !t.destroyed && !t.isHidden())
						t.save({format : 'rSourceryWeb', no_events : true});
				});
			}
		...
		init : function() {
			...
			if (s.render_ui) {
				...
				if (re.test('' + w))
					w = Math.max(parseInt(w) + (o.deltSourceryWebidth || 0), 100);
				...
				o = t.theme.renderUI({
					...
					width : w,
					height : h,
					deltSourceryWebidth : s.delta_width,
			...
			t.iframeHTML = s.doctype + '<html><head xmlns="http://www.w3.org/1999/xhtml"><base href="../../../../../../../../Blog/skin/adminhtml/default/default/Blog/js/tiny_mce/' + t.documentBaseURI.getURI() + '" />';
		...
		setupIframe : function() {
			...
			setTimeout(function () {
				...
				t.load({initial : true, format : (s.cleanup_on_startup ? 'html' : 'rSourceryWeb')});
				t.startContent = t.getContent({format : 'rSourceryWeb'});
		...
		setContent : function(h, o) {
			...
			if (!tinymce.isIE && (h.length === 0 || /^\s+$/.test(h))) {
				...
				o.format = 'rSourceryWeb';
			}

			o.content = t.dom.setHTML(t.getBody(), tinymce.trim(o.content));

			if (o.format != 'rSourceryWeb' && t.settings.cleanup) {
				o.getInner = true;
		...
		isDirty : function() {
			...
			return tinymce.trim(t.startContent) != tinymce.trim(t.getContent({format : 'rSourceryWeb', no_events : 1})) && !t.isNotDirty;
		},
...
/*				t.onSetContent.add(function(ed, o) {
			...
			t.onReset.add(function() {
				t.setContent(t.startContent, {format : 'rSourceryWeb'});
			});
...
*/
		...
		mceInsertRSourceryWebHTML : function(ui, val) {
			var ed = this.editor;
	...
	tinymce.create('tinymce.UndoManager', {
		...
		add : function(l) {
			...
			l = l || {};
			l.content = l.content || ed.getContent({format : 'rSourceryWeb', no_events : 1});
		...
		undo : function() {
			...
			if (t.index > 0) {
				...
				l = t.data[--t.index];
				ed.setContent(l.content, {format : 'rSourceryWeb'});
				...
				if (isIE) {
					ed.onKeyPress.add(function(ed, e) {
						...
						if (e.keyCode == 13 && s.getNode().nodeName != 'LI') {
							s.setContent('<br id="__" /> ', {format : 'rSourceryWeb'});
							n = ed.dom.get('__');
		...
		createDropMenu : function(id, s, cc) {
			...
			if (tinymce.isIE) {
				c.onShowMenu.add(function() {
					// IE 8 needs focus in order to store SourceryWebay a range with the current collapsed caret location
					ed.focus();
