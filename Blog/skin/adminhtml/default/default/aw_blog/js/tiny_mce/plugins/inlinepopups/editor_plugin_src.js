			...
			mdf = Event.add(id, 'mousedown', function(e) {
				...
				if (n.nodeName == 'A' || n.nodeName == 'a') {
					if (n.className == 'mceMax') {
						...
						DOM.setStyles(id + '_ifr', {width : vp.w - w.deltSourceryWebidth, height : vp.h - w.deltaHeight});
						DOM.addClass(id + '_wrapper', 'mceMaximized');
					} else if (n.className == 'mceMed') {
						// Reset to old size
						w.element.moveTo(w.oldPos.x, w.oldPos.y);
						w.element.resizeTo(w.oldSize.w, w.oldSize.h);
						w.iframeElement.resizeTo(w.oldSize.w - w.deltSourceryWebidth, w.oldSize.h - w.deltaHeight);
...
//			cp = DOM.getPos(t.editor.getContainer());
			...
			mu = Event.add(d, 'mouseup', function(e) {
				...
				we.resizeBy(dw, dh);
				sz = we.getSize();
				DOM.setStyles(id + '_ifr', {width : sz.w - w.deltSourceryWebidth, height : sz.h - w.deltaHeight});
				t._fixIELayout(id, 1);
