...
class Blog_Block_Blog extends Mage_Core_Block_Template
{
    public function getPosts()
   	{
		...
		$collection->setPageSize((int)Mage::getStoreConfig(Blog_Helper_Config::XML_BLOG_PERPAGE));
		...
		foreach ($collection as $item) 
		{
			...
			if(Mage::getStoreConfig(Blog_Helper_Config::XML_BLOG_USESHORTCONTENT) && strip_tags(trim($item->getShortContent()))){
			...
			}elseif ((int)Mage::getStoreConfig(Blog_Helper_Config::XML_BLOG_READMORE) != 0){
				$content = $item->getPostContent();
				if(strlen($content) >= (int)Mage::getStoreConfig(Blog_Helper_Config::XML_BLOG_READMORE))
				{
					$content = substr($content, 0, (int)Mage::getStoreConfig(Blog_Helper_Config::XML_BLOG_READMORE));
	...
	public function getTagsHtml($post){
		if (trim($post->getTags())){
			$this->setTemplate('Blog/line_tags.phtml');
			$this->setPost($post);
			return $this->toHtml();
		}
		return;
	...
	public function getRecent()
   	{
		if (Mage::getStoreConfig(Blog_Helper_Config::XML_RECENT_SIZE) != 0)
		{
			$collection = Mage::getModel('blog/blog')->getCollection()
			->addStoreFilter(Mage::app()->getStore()->getId())
			->setOrder('created_time ', 'desc');
			...
			Mage::getSingleton('blog/status')->addEnabledFilterToCollection($collection);
			$collection->setPageSize(Mage::getStoreConfig(Blog_Helper_Config::XML_RECENT_SIZE));
